...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const loginUser = (user) => {
    ...
    // Inicia Kanban Vazio para renderizar rápido
    initializeKanban();
  ...
  const initializeKanban = () => {
    ...
    // Não chama updateAttentionPanel aqui para não pesar o início
  };
  ...
  const renderDeliveredColumn = () => {
      ...
      // Limita a exibição inicial de entregues para não travar (Performance)
  ...
  // --- OTIMIZAÇÃO CRÍTICA DE PERFORMANCE (DEBOUNCE) ---
  // Evita que o painel pisque 100 vezes quando carrega 100 carros.
  // Ele espera 100ms sem novos dados para atualizar a tela.
  ...
  const listenToServiceOrders = () => {
    ...
    osRef.on('child_added', snapshot => {
      ...
      triggerAttentionUpdate(); // USA OTIMIZAÇÃO
    });
  ...
  // OTIMIZAÇÃO CLOUDINARY: Adicionado w_200 (miniaturas leves)
  ...
  logForm.addEventListener('submit', async (e) => { e.preventDefault(); const id = document.getElementById('logOsId').value; if(filesToUpload.length) for(let f of filesToUpload) { const r = await uploadFileToCloudinary(f); await db.ref(`serviceOrders/${id}/media`).push({url:r.url, type:f.type}); } await db.ref(`serviceOrders/${id}/logs`).push({ timestamp: new Date().toISOString(), user: currentUser.name, description: document.getElementById('logDescricao').value, parts: document.getElementById('logPecas').value, value: document.getElementById('logValor').value }); logForm.reset(); filesToUpload = []; postLogActions.style.display = 'block'; });
  ...
  openCameraBtn.addEventListener('click', ()=>{mediaInput.accept='image/*'; mediaInput.capture='camera'; mediaInput.click();}); openGalleryBtn.addEventListener('click', ()=>{mediaInput.accept='image/*,video/*,application/pdf'; mediaInput.removeAttribute('capture'); mediaInput.click();}); mediaInput.addEventListener('change', (e)=>{filesToUpload=[...e.target.files]; document.getElementById('fileName').textContent=`${filesToUpload.length} arqs`;});
